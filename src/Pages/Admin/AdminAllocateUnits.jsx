  ...
  const allocateMutation = useMutation({
    mutationFn: async (allocationData) => {
      ...
      if (!latest || !latest.unit_value) {
        throw new Error('No unit valuation available.');
      }
    ...
    onSuccess: (data) => {
      ...
      setSuccess(`Allocated ${data.units.toFixed(4)} units at $${data.unitValue.toFixed(4)}/unit`);
      setComputedUnits(data.units);
      setFormData({ member_id: "", tx_date: new Date().toISOString().split('T')[0], cash_amount: "" });
