-- LEGACY SCHEMA (MOVE/REFERENCE ONLY)
-- NOTE: This SQL file is preserved for historical/migration purposes only.
-- The runtime application has migrated to a normalized model. DO NOT use these legacy
-- tables in runtime code. The current live model and RPCs are:
--   tables: members, member_monthly_balances, member_dues, org_balance_history
--   views: v_member_positions_as_of, v_member_growth
--   RPCs: api_get_dashboard(as_of_date), api_get_member_timeline(member_id_in)
--
-- This file originally lived under src/database and has been moved to database/legacy/
-- to avoid accidental runtime references. Keep as a migration artifact.

-- Member Data Database Schema (legacy)
-- This file contains all the SQL commands to create tables for the older member data storage
...
-- Member Unit Values Table - Track individual member unit holdings (legacy)
...
-- Member Portfolio Data Table - Track individual holdings and performance (legacy)
...
-- Club Values Table - Track overall club performance and metrics (legacy)
...
-- Payment History Table - Detailed payment tracking (legacy)
...
-- Indexes for better performance (legacy)
